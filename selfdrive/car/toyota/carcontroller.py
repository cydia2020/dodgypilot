...
class CarController:
  ...
  def update(self, CC, CS):
    ...
    filtered_raw_accel_cmd = self.accel_filter.x
    # calculate and clip pcm_accel_cmd
    if not CS.out.gasPressed:
      pcm_accel_cmd = clip(filtered_raw_accel_cmd + pcm_neutral_force, CarControllerParams.ACCEL_MIN, _accel_max)
    else:
      pcm_accel_cmd = 0.
    ...
    if (self.frame % 3 == 0 and self.CP.openpilotLongitudinalControl) or pcm_cancel_cmd:
      ...
      if pcm_cancel_cmd and self.CP.carFingerprint in (CAR.LEXUS_IS, CAR.LEXUS_RC):
        ...
        can_sends.append(create_accel_command(self.packer, pcm_accel_cmd, pcm_cancel_cmd, self.standstill_req, lead, CS.acc_type, adjust_distance, fcw_alert, lead_vehicle_stopped, filtered_raw_accel_cmd, should_compensate, acc_msg))
