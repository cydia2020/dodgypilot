...
class Controls:
  ...
  def update_events(self, CS):
    ...
    if self.is_fcw_enabled and (planner_fcw or model_fcw) and not (self.CP.notCar and self.joystick_mode):
  ...
  def publish_carControl(self, CS, CC, lac_log):
    ...
    if len(speeds):
      CC.cruiseControl.resume = self.enabled and CS.cruiseState.standstill and speeds[-1] > 0.4
    ...
    CC.hudControl.rightLaneVisible = bool(right_lane_visible) and CS.vEgo > 125/9
    CC.hudControl.leftLaneVisible = bool(left_lane_visible) and CS.vEgo > 125/9
    ...
    if self.AM.current_alert:
      ...
      hudControl.audibleAlert = self.AM.current_alert.audible_alert
